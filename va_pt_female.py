...
def record_audio(ask=""):
    with sr.Microphone() as source:
        ...
        print("A processar a sua resposta.")
        voice_data = ''
        try :
            voice_data= r.recognize_google(audio)#convert audio to text
                ...
                engine_speak('Desculpe não consegui entender.')
        except sr.RequestError:
                engine_speak("Ocorreu um erro")#error : recognizer is not connected
...
def engine_speak(audio_string):
    ...
    tts = gTTS(text=audio_string , lang='pt')#text to speech(voice)
...
def respond(voice_data):
    # 1) if got greeting
    if there_exists(['olá','hello','hola','boas','oi',"ola",]):
        greetings=["Olá, como possoa ajuda-lo?" +person_obj.name, "Olá, como possoa ajuda-lo??" +person_obj.name, "Olá, como possoa ajuda-lo?"+person_obj.name]
...
while(1):
    voice_data = record_audio("A ouvir..") # get the voice input
    print("Done")
    print("Q:", voice_data)
